...
Ο κώδικας των υλοποιήσεων μαζί με την main() βρίσκονται στο test.cpp.
Στο directory includes/utils υπάρχουν όλα τα απαραίτητα declarations/implementations για συναρτήσεις που χρησιμοποιούνται από το test.cpp. Με χρήση της CMake δημιουργείται το library Utils που χρησιμοποιείται στο τελικό εκτελέσιμο.
...
όπου το steiner_count είναι ο μέγιστος αριθμός των steiner σημείων που μπορούν να προστεθούν στην τριγωνοποίηση μέσω της brute_force().
Επειδή η πολυπλοκότητα της brute_force() είναι εκθετική, πρέπει το steiner_count να έχει μικρή τιμή (max 11) αλλά για αυτά θα βρει τη βέλτιστη λύση, αν υπάρχει. 
