...
void apply_best_sequence(CDT& cdt, Polygon_2& polygon,  const vector<string>& sequence, vector<Point_2>& steiner, map<Point, int>& index) {
	for (const string& step : sequence) {
		...
		if (step == "insert_circumcenter") {
			to_insert = insert_circumcenter(cdt, polygon);
		} else if (step == "insert_midpoint") {
			to_insert = insert_midpoint(cdt, polygon);
		} else if (step == "insert_projection") {
			to_insert = insert_projection(cdt, polygon);
		} else if(step == "insert_centroid") {
			to_insert = insert_centroid(cdt, polygon);
		} else if(step == "insert_adjacent") {
			to_insert = insert_adjacent(cdt, polygon);
		}
		steiner.emplace_back(to_insert);
		index[to_insert] = index.size();
		cout << "Applied " << step << "\n";
	}
}
...
void try_combinations(CDT& cdt, Polygon_2& polygon, int max_depth, int current_depth, int& min_obtuse_angles, vector<string>& best_sequence, vector<string>& current_sequence, int& min_steiner_points, map<Point, int>& index) {
	...
	try_combinations(cdt, polygon, max_depth, current_depth + 1, min_obtuse_angles, best_sequence, current_sequence, min_steiner_points, index);
...
void brute_force_steiner_insertion(CDT& cdt, int max_steiner_points, Polygon_2& polygon, vector<Point_2>& steiner, map<Point, int>& index) {
	...
	try_combinations(cdt, polygon, max_steiner_points, 0, min_obtuse_angles, best_sequence, current_sequence, min_steiner_points, index);
	...
	if (!best_sequence.empty()) {
		...
		cout << endl;
		apply_best_sequence(cdt, polygon, best_sequence, steiner, index);
		cout << "\n";
	} else {
